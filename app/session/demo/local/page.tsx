...
export default function LocalRecognitionPage() {
  ...
  const chooseFolder = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setFiles(picked); setCurrent(picked[0] || null); setSelectedFiles(new Set(picked[0] ? [fileKey(picked[0])] : [])); setAssignments({}); setResult(null); setError("");
  };
  ...
  return <main style={{ minHeight: "100vh", overflow: "auto", background: "#f3f5f2", padding: 24 }}>
    ...
    <div style={{ maxWidth: 1180, margin: "auto", display: "grid", gridTemplateColumns: "270px minmax(360px,1fr) 350px", gap: 14 }}>
      <section style={card}>
        ...
        <label style={folderButton}>选择图片文件夹<input type="file" accept="image/*" multiple {...({ webkitdirectory: "" } as object)} onChange={chooseFolder} style={{ display: "none" }} /></label>
        <p style={hint}>已读取 {files.length} 张，已选择 {selectedFiles.size} 张。浏览器只获得本次授权的文件。</p>
        ...
        <div style={{ maxHeight: 560, overflow: "auto" }}>{files.map((f, i) => { const key = fileKey(f); return <div key={`${key}-${i}`} style={{ display: "flex", alignItems: "center", background: current === f ? "#e8f4ec" : "transparent", borderRadius: 5 }}><input aria-label="选择素材" type="checkbox" checked={selectedFiles.has(key)} onChange={() => setSelectedFiles(old => toggleSet(old, key))} /><button onClick={() => { setCurrent(f); setResult(null); }} style={{ ...fileButton, color: current === f ? "#23754f" : "#55615a" }}>{f.webkitRelativePath || f.name}</button><span style={{ fontSize: 9, color: "#31805b", paddingRight: 4 }}>{assignments[key]?.length || ""}</span></div> })}</div>
      </section>
      <section style={card}>
        <h3 style={heading}>2. 当前图片</h3>
        <div style={{ height: 420, background: "#202421", display: "grid", placeItems: "center", borderRadius: 8, overflow: "hidden" }}>{preview ? <img src={preview} alt="本地预览" style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain" }} /> : <span style={{ color: "#9ba39e" }}>请先选择文件夹</span>}</div>
        <p style={{ fontSize: 12, color: "#68736c" }}>{current?.name || "未选择图片"}</p>
        ...
        <button onClick={recognize} disabled={!current || loading} style={{ ...folderButton, border: 0, width: "100%", opacity: !current || loading ? .5 : 1 }}>{loading ? "VITA 正在识别…" : "识别并推荐标签"}</button>
