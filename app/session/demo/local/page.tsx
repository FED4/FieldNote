...
export default function LocalRecognitionPage() {
  ...
  const transcribeAudio = async (audio: File) => {
    ...
    try {
      ...
      setVoiceContext(old => [old.trim(), body.transcript].filter(Boolean).join("\n"));
  ...
  return <main style={{ minHeight: "100vh", overflow: "auto", background: "#f3f5f2", padding: 24 }}>
    ...
    <div style={{ maxWidth: 1180, margin: "auto", display: "grid", gridTemplateColumns: "270px minmax(360px,1fr) 350px", gap: 14 }}>
      ...
      <section style={card}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 7, color: "#89938d", fontSize: 9, margin: "5px 0 10px" }}><span style={{ flex: 1 }}>{voiceContext.length} 字 · 随图片一起用于标签推理</span><label style={{ ...smallButton, cursor: asrLoading ? "wait" : "pointer", opacity: asrLoading ? .55 : 1 }}>{asrLoading ? "腾讯云识别中…" : "＋ 导入语音片段"}<input disabled={asrLoading} type="file" accept="audio/*,.wav,.mp3,.m4a,.aac,.ogg,.webm" style={{ display: "none" }} onChange={e => { const file = e.target.files?.[0]; if (file) void transcribeAudio(file); e.target.value = ""; }} /></label><button onClick={() => setVoiceContext("")} style={{ border: 0, background: "none", color: "#557263", fontSize: 9 }}>清空</button></div>
