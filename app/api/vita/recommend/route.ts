...
export async function POST(request: NextRequest) {
  ...
  const { imageDataUrl, systemPrompt, voiceContext } = await request.json() as { imageDataUrl?: string; systemPrompt?: string; voiceContext?: string };
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: [
          { type: "text", text: `${systemPrompt || "识别现场考察图片并推荐结构化标签。"}${transcriptContext}${formatInstruction}` },
