...
export async function POST(request: NextRequest) {
  ...
  const formatInstruction = `\n\n请只返回 JSON，不要使用 Markdown：{"summary":"结合媒体与讨论的简述","tags":[{"facet":"厂房|工段|地点|设备|部件|活动|工艺|状态|对象|问题|材料|文档类型|人员|其他","name":"标签名称","confidence":0.0,"reason":"说明依据来自图片、视频、语音、原始目录或组合"}]}`;
  ...
  try {
    ...
    return NextResponse.json({ result: { summary: answer, tags: [] }, raw: answer });
  }
}
